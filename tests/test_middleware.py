...
class TestProblem:
    ...
    def test_to_bytes(self):
        ...
        assert (
            problem.to_bytes()
            == b'{"type":"about:blank","title":"Internal Server Error","status":500}'
        )  # noqa
...
async def test_get_exception_handler():
    ...
    assert (
        resp.body
        == b'{"exc_type":"ValueError","type":"about:blank","title":"Unexpected Server Error","status":500,"detail":"test error"}'
    )  # noqa
...
async def test_get_exception_handler_debug():
    ...
    resp = await handler(
        request=Request(scope={"type": "http", "app": FastAPI(debug=True)}),
        exc=ValueError("test error"),
    )
...
def test_register():
    app = FastAPI()
    assert len(app.exception_handlers) == 3  # default handlers
    ...
    for key in app.exception_handlers:
        # Ignore the websocket_request_validation_exception_handler
        if key is not fastapi.exceptions.WebSocketRequestValidationError:
